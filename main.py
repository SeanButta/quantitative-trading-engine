...
# ---------------------------------------------------------------------------
# Structured logging — must come before any other imports that log
# ---------------------------------------------------------------------------
from middleware import configure_structured_logging, attach_middleware
configure_structured_logging()

from models import (
    Base, Project, Strategy, Run,
    OptionsRefreshJob, SignalReadingJob, PortfolioAnalysisJob,
...
# ---------------------------------------------------------------------------
# Rate Limiting (slowapi — degrades gracefully if not installed)
# ---------------------------------------------------------------------------
try:
...
# CORS — locked to known origins via env var (falls back to permissive for local dev)
...
# Database
# ---------------------------------------------------------------------------
# SQLite for dev; set DATABASE_URL=postgresql://... for production.
# PostgreSQL notes:
#   - Remove connect_args (not valid for psycopg2/asyncpg)
...
# ===========================================================================
# These endpoints mirror the localStorage schema from the frontend but store
# data server-side, scoped per authenticated user.  The frontend checks auth
# status on load and routes to these endpoints when a session exists, or falls
# back to localStorage when running unauthenticated (demo mode).
# ===========================================================================
...
class AddPositionRequest(BaseModel):
    ...
    symbol: str
    side: str = "buy"
    qty: Optional[float] = None        # equity
    entry_price: Optional[float] = None
    contracts: Optional[int] = None    # options
...
def list_portfolios(current_user: dict = Depends(get_current_user)):
    ...
    db = SessionLocal()
    try:
        rows = db.query(PaperPortfolio).filter(
            PaperPortfolio.user_id == current_user["sub"]
        ).order_by(PaperPortfolio.created_at).all()
        return [
            {
                "id": p.id,
                "name": p.name,
...
@app.post("/paper/portfolios", status_code=201)
def create_portfolio(req: CreatePortfolioRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        p = PaperPortfolio(
            ...
            name=req.name.strip() or "My Portfolio",
        ...
        db.commit()
        db.refresh(p)
        return {"id": p.id, "name": p.name, "starting_cash": p.starting_cash, "cash": p.cash}
    except Exception:
        db.rollback()
        logger.exception("Failed to create portfolio")
        raise HTTPException(status_code=500, detail="Could not create portfolio")
    finally:
        db.close()


@app.delete("/paper/portfolios/{portfolio_id}")
def delete_portfolio(portfolio_id: str, current_user: dict = Depends(get_current_user)):
    """Delete a portfolio and all its positions/journal entries."""
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
...
@app.patch("/paper/portfolios/{portfolio_id}/cash")
def update_cash(portfolio_id: str, req: UpdateCashRequest, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
            PaperPortfolio.id == portfolio_id,
        ...
        p.cash = req.cash
        db.commit()
        return {"id": portfolio_id, "cash": p.cash}
    except HTTPException:
        raise
    except Exception:
        db.rollback()
        raise HTTPException(status_code=500, detail="Cash update failed")
    finally:
        db.close()
...
def list_positions(portfolio_id: str, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
            PaperPortfolio.id == portfolio_id,
...
@app.post("/paper/portfolios/{portfolio_id}/positions", status_code=201)
def add_position(portfolio_id: str, req: AddPositionRequest, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
            PaperPortfolio.id == portfolio_id,
        ...
        raise HTTPException(status_code=500, detail="Could not add position")
    finally:
        db.close()


@app.delete("/paper/portfolios/{portfolio_id}/positions/{position_id}")
def close_position(portfolio_id: str, position_id: str, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
            PaperPortfolio.id == portfolio_id,
...
@app.get("/paper/portfolios/{portfolio_id}/journal")
def get_journal(portfolio_id: str, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
            PaperPortfolio.id == portfolio_id,
        ...
        return [
            ...
            for j in sorted(p.journal, key=lambda x: x.date or datetime.min, reverse=True)
        ]
    finally:
        db.close()


@app.post("/paper/portfolios/{portfolio_id}/journal", status_code=201)
def add_journal_entry(portfolio_id: str, req: AddJournalEntryRequest, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
            PaperPortfolio.id == portfolio_id,
        ...
        raise HTTPException(status_code=500, detail="Could not add journal entry")
    finally:
        db.close()
...
@app.post("/paper/portfolios/{portfolio_id}/import")
def bulk_import(portfolio_id: str, req: BulkImportRequest, current_user: dict = Depends(get_current_user)):
    """
    One-shot import of localStorage paper trading data to the backend.
    Called by the frontend when a user first logs in with existing local data.
    """
    db = SessionLocal()
    try:
        p = db.query(PaperPortfolio).filter(
        ...
        for pos_data in req.positions:
            pos = PaperPosition(
                id=pos_data.get("id") or str(uuid.uuid4()),
                portfolio_id=portfolio_id,
                type=pos_data.get("type", "equity"),
                symbol=(pos_data.get("symbol") or "").upper(),
                side=pos_data.get("side", "buy"),
                qty=pos_data.get("qty"),
                entry_price=pos_data.get("entryPrice") or pos_data.get("entry_price"),
                contracts=pos_data.get("contracts"),
                strike=pos_data.get("strike"),
                expiry=pos_data.get("expiry"),
                entry_premium=pos_data.get("entryPremium") or pos_data.get("entry_premium"),
                underlying_at_entry=pos_data.get("underlyingAtEntry") or pos_data.get("underlying_at_entry"),
                iv=pos_data.get("iv"),
        ...
        for jnl_data in req.journal:
            j = PaperJournal(
                id=jnl_data.get("id") or str(uuid.uuid4()),
                portfolio_id=portfolio_id,
                action=jnl_data.get("action", ""),
                type=jnl_data.get("type", "equity"),
                symbol=(jnl_data.get("symbol") or "").upper(),
                strike=jnl_data.get("strike"),
                expiry=jnl_data.get("expiry"),
                qty=jnl_data.get("qty"),
                contracts=jnl_data.get("contracts"),
                price=jnl_data.get("price"),
                total=jnl_data.get("total"),
                pnl=jnl_data.get("pnl"),
                notes=jnl_data.get("notes", ""),
            )
            db.add(j)
            imported_jnl += 1
        ...
        db.commit()
        return {
            "imported_positions": imported_pos,
            "imported_journal_entries": imported_jnl,
            "cash": p.cash,
        }
    except HTTPException:
        raise
...
def list_watchlists(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return [{"id": w.id, "name": w.name, "symbols": w.symbols or []} for w in rows]
    finally:
        db.close()


@app.post("/watchlists", status_code=201)
def create_watchlist(req: CreateWatchlistRequest, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        w = Watchlist(
            id=str(uuid.uuid4()),
...
def update_watchlist(watchlist_id: str, req: UpdateWatchlistRequest, current_user: dict = Depends(get_current_user)):
    db = SessionLocal()
    try:
        ...
        if req.symbols is not None:
            w.symbols = [s.upper() for s in req.symbols]
        db.commit()
        return {"id": w.id, "name": w.name, "symbols": w.symbols}
    except HTTPException:
        raise
...
def admin_stats(current_user: dict = Depends(get_current_user)):
    """Platform statistics. Requires admin tier."""
    if current_user.get("tier") != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    db = SessionLocal()
    try:
        user_count       = db.query(User).count()
        portfolio_count  = db.query(PaperPortfolio).count()
        position_count   = db.query(PaperPosition).count()
        journal_count    = db.query(PaperJournal).count()
...
def admin_cache_clear(prefix: Optional[str] = None, current_user: dict = Depends(get_current_user)):
...
@app.get("/health")
def health_check():
    """Liveness probe. Returns DB status and cache backend."""
    db_ok = False
    try:
        db = SessionLocal()
