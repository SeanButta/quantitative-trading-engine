...
function Stat({label,value,color,sub}) {
  const C = useC();
  return <Card><Lbl>{label}</Lbl><div style={mono(20,color||C.grn,800)}>{value}</div>{sub&&<div style={{...mono(9,C.mut),marginTop:3}}>{sub}</div>}</Card>;
}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div style={{borderRadius:16,border:`1px solid ${C.grn}30`,background:C.grnBg,padding:22}}>
        ...
        <div style={mono(26,C.headingTxt,800)}>Research Platform</div>
        ...
        <Card>
          ...
          {[["01","Create Project","Name, symbols, date range",true],["02","Ingest Data","yfinance → Parquet store",true],["03","Compute Features","Returns, vol, ATR, PCA…",true],["04","Build Strategy","Select signals + parameters",false],["05","Run Backtest","Walk-forward, no lookahead",false],["06","Validate","t-test · BH · permutation",false],["07","Read Report","Full markdown research doc",false]].map(([n,l,s,d])=>(
            <div key={n} style={{display:"flex",alignItems:"center",gap:12,padding:"7px 10px",borderRadius:8,marginBottom:4,background:d?C.grnBg:"transparent",border:`1px solid ${d?C.grn+"25":C.bdr}`}}>
              ...
              <div style={{flex:1}}><div style={mono(11,d?C.headingTxt:C.mut,600)}>{l}</div><div style={mono(9,C.mut)}>{s}</div></div>
      ...
      <Card>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:14}}>
          {[["Fee (bps)","1.0"],["Slippage (bps)","2.0"],["Risk-Free Rate","3.0%"]].map(([l,v])=>(
            <div key={l}><Lbl>{l}</Lbl><input defaultValue={v} style={{...mono(12,C.txt),width:"100%",padding:"7px 11px",borderRadius:8,background:C.dim,border:`1px solid ${C.bdr}`,outline:"none",boxSizing:"border-box"}}/></div>
          ))}
        </div>
        <button onClick={go} disabled={running} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 20px",borderRadius:8,border:"none",cursor:running?"not-allowed":"pointer",background:running?C.dim:C.grn,color:running?C.mut:"#000",...mono(12,running?C.mut:"#000",700),transition:"all .15s"}}>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      {sig&&<Card accent={sig.color}>
        <div style={mono(14,C.headingTxt,700)}>{sig.label}</div>
      ...
      <Card accent={C.grn}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16}}>
          <div>
            ...
            <div style={mono(24,C.headingTxt,800)}>SPY Momentum Alpha</div>
