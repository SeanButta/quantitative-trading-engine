...
function DailyBriefView() {
  ...
  const generateNarrative = () => {
    ...
    fetch("/api/daily-brief/narrative",{method:"POST"}).then(r=>r.json()).then(d=>{setNarrative(d);setNarrativeLoading(false);}).catch(()=>setNarrativeLoading(false));
  };
  const cc = v => v==null?C.mut:v>=0?C.grn:C.red;
  const fp = v => v!=null?(v>=0?"+":"")+v.toFixed(1)+"%":"—";
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {data ? (<>
        {/* AI Narrative */}
        {narrative?.narrative ? (
          <Section accent={C.pur} title="AI MORNING NOTE">
            <div style={{...mono(11,C.txt),lineHeight:1.8,whiteSpace:"pre-wrap"}}>{narrative.narrative}</div>
            <div style={{...mono(8,C.mut),marginTop:8}}>Generated by {narrative.model} · {narrative.total_tokens} tokens · ${narrative.cost_usd?.toFixed(4)}</div>
          </Section>
        ) : (
          <div style={{display:"flex",justifyContent:"flex-end"}}>
            <button onClick={generateNarrative} disabled={narrativeLoading}
              style={{...mono(10,narrativeLoading?C.mut:C.pur,700),padding:"6px 14px",borderRadius:8,
                border:`1px solid ${C.pur}40`,background:C.pur+"10",cursor:narrativeLoading?"not-allowed":"pointer"}}>
              {narrativeLoading ? "Generating..." : "✨ Generate AI Morning Note"}
            </button>
          </div>
        )}
